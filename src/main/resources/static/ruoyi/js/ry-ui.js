         ...
         // 导入word
            importWord: function(formId) {
            	...
            	layer.open({
            		...
            		btn1: function(index, layero){
            			var file = layero.find('#file').val();
            			if (file == '' || (!$.common.endWith(file, '.doc') && !$.common.endWith(file, '.docx'))){
            				$.modal.msgWarning("请选择后缀为 “doc”或“docx”的Word文件。");
            				return false;
            			}
            			var index = layer.load(2, {shade: false});
            			$.modal.disable();
        ...
        operate: {
            ...
            look: function(id) {
            	...
            	if($.common.isEmpty(id) && table.options.type == table_type.bootstrapTreeTable) {
            	    ...
            	    $.modal.openLook("查看" + table.options.modalName, $.operate.lookUrl(id));
            	}
            },
            ...
            lookUrl: function(id) {
            	var url = "/404.html";
            	if ($.common.isNotEmpty(id)) {
            	    url = table.options.lookUrl.replace("{id}", id);
            	} else {
            	    var id = $.common.isEmpty(table.options.uniqueId) ? $.table.selectFirstColumns() : $.table.selectColumns(table.options.uniqueId);
            	    if (id.length == 0) {
            			$.modal.alertWarning("请至少选择一条记录");
